...
import { payloadCreate } from "tests/resources/payload/payload-create";
import {
  payloadLogin,
  payloadLoginFailed,
...
describe("Validate POST Users", () => {
  ...
  it("should validate /login failed unsuccessfull", async () => {
    ...
    printResponse(response);
    expect(response.statusCode).equal(responseCode.badRequest);
  ...
  it("should validate /users create successfull", async () => {
    ...
    const response = await methodPOST("users", payloadCreate(value), {});
    printResponse(response);
    expect(response.statusCode).equal(responseCode.success_other);
